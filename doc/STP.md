...
| 3 | <a id="parse_args_default_port_baud_load_when_not_supplied"></a>`parse_args_default_port_baud_load_when_not_supplied` | `LLR-MAIN-02` | Call `parse_args()` with only the two mandatory positional arguments and verify that `cfg.port == 1234`, `cfg.baud == 115200`, and `cfg.load == false`. |
...
| # | Test | Verifies | Purpose |
| - | ---- | -------- | ------- |
| 1 | <a id="updi_open_sets_8n2_raw_half_duplex_via_termios"></a>`updi_open_sets_8n2_raw_half_duplex_via_termios` | `LLR-UPDI-01` | Open a PTY pair, call `updi_open()` on the slave side, and inspect the resulting `termios` structure to confirm 8 data bits, no parity, 2 stop bits (`CSTOPB`), and raw mode (`cfmakeraw()`). |
...
| 3 | <a id="updi_open_asserts_break_at_300_baud_before_synch"></a>`updi_open_asserts_break_at_300_baud_before_synch` | `LLR-UPDI-02` | Verify that `updi_open()` temporarily sets the baud rate to `UPDI_BREAK_BAUD` (300) and transmits a 0x00 byte before transmitting 0x55, confirming the BREAK-before-SYNCH ordering. |
| 4 | <a id="updi_open_restores_session_baud_after_break"></a>`updi_open_restores_session_baud_after_break` | `LLR-UPDI-02` | Verify that `updi_open()` restores the configured session baud rate after the BREAK condition and before transmitting the SYNCH character (0x55). |
| 5 | <a id="updi_open_retries_break_synch_3_times_on_no_ack"></a>`updi_open_retries_break_synch_3_times_on_no_ack` | `LLR-UPDI-03` | Simulate a target that never returns ACK (0x40) and verify that `updi_open()` repeats the BREAK+SYNCH sequence exactly 3 times before giving up. |
| 6 | <a id="updi_open_returns_minus1_after_3_consecutive_link_failures"></a>`updi_open_returns_minus1_after_3_consecutive_link_failures` | `LLR-UPDI-03` | After 3 failed BREAK+SYNCH attempts with no ACK, verify that `updi_open()` returns -1. |
| 7 | <a id="updi_mem_read_splits_request_larger_than_256_bytes"></a>`updi_mem_read_splits_request_larger_than_256_bytes` | `LLR-UPDI-04` | Request a read of 512 bytes and verify that `updi_mem_read()` issues two consecutive 256-byte REPEAT+LD bursts rather than a single oversized transfer. |
| 8 | <a id="updi_mem_read_uses_repeat_ld_auto_increment_sequence"></a>`updi_mem_read_uses_repeat_ld_auto_increment_sequence` | `LLR-UPDI-04` | Capture the bytes written to the UART and verify that each block begins with the UPDI REPEAT opcode followed by a LD-auto-increment command, confirming the correct UPDI framing. |
| 9 | <a id="updi_mem_read_calls_select_with_100ms_timeout_before_read"></a>`updi_mem_read_calls_select_with_100ms_timeout_before_read` | `LLR-UPDI-05` | Using a mock `select()` that records its timeout argument, verify that `updi_mem_read()` passes a `timeval` of 0 seconds and 100,000 microseconds before every `read()` call. |
| 10 | <a id="updi_mem_read_returns_minus1_on_select_timeout"></a>`updi_mem_read_returns_minus1_on_select_timeout` | `LLR-UPDI-05` | Inject a mock `select()` that returns 0 (timeout) and verify that `updi_mem_read()` immediately returns -1 without calling `read()`. |
| 11 | <a id="updi_mem_write_returns_0_on_success"></a>`updi_mem_write_returns_0_on_success` | `LLR-UPDI-06` | Call `updi_mem_write()` targeting a SRAM address with a loopback PTY that echoes the expected ACK and verify the return value is 0. |
| 12 | <a id="updi_mem_write_returns_minus1_on_updi_nak"></a>`updi_mem_write_returns_minus1_on_updi_nak` | `LLR-UPDI-06` | Inject a mock that returns a UPDI NAK instead of ACK and verify that `updi_mem_write()` returns -1. |
| 13 | <a id="updi_nvm_write_flash_rejects_unaligned_address"></a>`updi_nvm_write_flash_rejects_unaligned_address` | `LLR-UPDI-07` | Call `updi_nvm_write_flash()` with a `word_addr` that is not a multiple of 512 and verify the function returns -1 without issuing any UART traffic. |
| 14 | <a id="updi_nvm_write_flash_rejects_non_multiple_of_512_length"></a>`updi_nvm_write_flash_rejects_non_multiple_of_512_length` | `LLR-UPDI-07` | Call `updi_nvm_write_flash()` with a `len` that is not a non-zero multiple of 512 and verify the function returns -1. |
| 15 | <a id="updi_nvm_write_flash_nvmprog_poll_timeout_returns_minus1"></a>`updi_nvm_write_flash_nvmprog_poll_timeout_returns_minus1` | `LLR-UPDI-07` | Inject a mock target that never clears the NVMPROG BUSY bit and verify that `updi_nvm_write_flash()` returns -1 after 100 ms. |
| 16 | <a id="updi_nvm_write_flash_per_page_busy_timeout_returns_minus1"></a>`updi_nvm_write_flash_per_page_busy_timeout_returns_minus1` | `LLR-UPDI-07` | Inject a mock target that never clears the NVM BUSY bit after a page write and verify that `updi_nvm_write_flash()` returns -1 after 20 ms. |
| 17 | <a id="updi_halt_returns_0_when_stopped_bit_set_within_50ms"></a>`updi_halt_returns_0_when_stopped_bit_set_within_50ms` | `LLR-UPDI-08` | Inject a mock `ASI_SYS_STATUS` that sets the STOPPED bit (bit 3) on the first poll and verify that `updi_halt()` returns 0. |
| 18 | <a id="updi_halt_returns_minus1_after_50ms_timeout"></a>`updi_halt_returns_minus1_after_50ms_timeout` | `LLR-UPDI-08` | Inject a mock `ASI_SYS_STATUS` that never sets the STOPPED bit and verify that `updi_halt()` returns -1 after 50 polling intervals. |
| 19 | <a id="updi_step_issues_asi_step_command_and_confirms_stopped_bit"></a>`updi_step_issues_asi_step_command_and_confirms_stopped_bit` | `LLR-UPDI-09` | Verify that `updi_step()` writes the ASI single-step command and then reads `ASI_SYS_STATUS` to confirm the STOPPED bit is set before returning 0. |
| 20 | <a id="updi_run_clears_halt_request_and_verifies_stopped_cleared"></a>`updi_run_clears_halt_request_and_verifies_stopped_cleared` | `LLR-UPDI-10` | Verify that `updi_run()` clears the ASI halt request and confirms the STOPPED bit is no longer set in `ASI_SYS_STATUS` before returning 0. |
| 21 | <a id="updi_mem_read_callable_while_cpu_running_no_halt_issued"></a>`updi_mem_read_callable_while_cpu_running_no_halt_issued` | `LLR-UPDI-11` | With the mock target in RUNNING state (STOPPED bit clear), call `updi_mem_read()` and verify that no halt request is written to `ASI_SYS_CTRL`. |
| 22 | <a id="updi_console_poll_returns_pending_bytes_without_halting"></a>`updi_console_poll_returns_pending_bytes_without_halting` | `LLR-UPDI-12` | Inject a mock avrOS software UART buffer containing 4 bytes and verify that `updi_console_poll()` returns 4 and copies the bytes to the output buffer without calling `updi_halt()`. |
| 23 | <a id="updi_console_poll_returns_0_when_output_buffer_empty"></a>`updi_console_poll_returns_0_when_output_buffer_empty` | `LLR-UPDI-12` | With the mock avrOS software UART buffer empty, verify that `updi_console_poll()` returns 0. |
...
| `LLR-UPDI-01` | `updi` | `HLR-006` | `updi_open_sets_8n2_raw_half_duplex_via_termios`, `updi_open_returns_minus1_on_device_open_failure` |
| `LLR-UPDI-02` | `updi` | `HLR-006`, `HLR-036` | `updi_open_asserts_break_at_300_baud_before_synch`, `updi_open_restores_session_baud_after_break` |
| `LLR-UPDI-03` | `updi` | `HLR-036` | `updi_open_retries_break_synch_3_times_on_no_ack`, `updi_open_returns_minus1_after_3_consecutive_link_failures` |
| `LLR-UPDI-04` | `updi` | `HLR-007` | `updi_mem_read_splits_request_larger_than_256_bytes`, `updi_mem_read_uses_repeat_ld_auto_increment_sequence` |
...
| `LLR-UPDI-08` | `updi` | `HLR-010`, `HLR-037` | `updi_halt_returns_0_when_stopped_bit_set_within_50ms`, `updi_halt_returns_minus1_after_50ms_timeout` |
| `LLR-UPDI-09` | `updi` | `HLR-010` | `updi_step_issues_asi_step_command_and_confirms_stopped_bit` |
| `LLR-UPDI-10` | `updi` | `HLR-010` | `updi_run_clears_halt_request_and_verifies_stopped_cleared` |
| `LLR-UPDI-11` | `updi` | `HLR-011` | `updi_mem_read_callable_while_cpu_running_no_halt_issued` |
