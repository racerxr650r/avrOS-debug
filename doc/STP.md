...
| 3 | <a id="updi_open_asserts_two_breaks_then_synch"></a>`updi_open_asserts_two_breaks_then_synch` | `LLR-UPDI-02` | Verify that `updi_open()` transmits two consecutive 0x00 bytes (the UPDI BREAK pattern) followed by the SYNCH character 0x55 at the session baud rate, with no intervening kernel baud-rate switch. |
| 4 | <a id="updi_open_issues_ldcs_statusb_after_synch"></a>`updi_open_issues_ldcs_statusb_after_synch` | `LLR-UPDI-03` | Capture the bytes written by `updi_open()` after SYNCH and verify the link-probe opcode is `LDCS ASI_STATUSB` (`0x80 | 0x01 = 0x81`), which clears the PESIG bit set by BREAK and confirms target responsiveness. |
| 5 | <a id="updi_open_restores_session_baud_after_break"></a>`updi_open_restores_session_baud_after_break` | `LLR-UPDI-02` | Verify that `updi_open()` leaves the configured session baud rate (B115200) in effect after a successful BREAK+SYNCH sequence — i.e. it does not corrupt the termios baud setting. |
| 6 | <a id="updi_open_retries_break_synch_3_times_on_no_ack"></a>`updi_open_retries_break_synch_3_times_on_no_ack` | `LLR-UPDI-03` | Simulate a target that never responds to LDCS and verify that `updi_open()` transmits the BREAK+BREAK+SYNCH+LDCS sequence exactly 3 times (six 0x00 BREAK bytes total) before giving up. |
| 7 | <a id="updi_open_returns_minus1_after_3_consecutive_link_failures"></a>`updi_open_returns_minus1_after_3_consecutive_link_failures` | `LLR-UPDI-03` | After 3 failed BREAK+SYNCH+LDCS attempts with no response, verify that `updi_open()` returns -1. |
