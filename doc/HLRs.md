...
*   <a id="HLR-058"></a>**HLR-058: Extended-Remote Lifecycle Packets.**
    The application shall support the `target extended-remote` connection mode by implementing the `vRun;<arg>;<arg>...` packet (treat the first argument as a path to a new `.elf` file, swap it into the existing `AppConfig`, re-run ELF parsing, re-build the FSM context, and reply with a `T05` stop at the new reset vector), the `vAttach;<pid>` packet (reply `OK` followed by the standard stop-reply; `pid` is informational only on a single-target stub), and the `vKill;<pid>` packet (call `updi_run()` to release the target, close the GDB client fd, and return to the listen state without exiting the server process). Existing `D` (detach) and `k` (kill) handlers shall remain operational; `k` shall continue to mean process-exit so that scripts that hard-stop the server keep working. The `qSupported` feature string shall be extended with `multiprocess+;vRun+;vAttach+;vKill+` and the `multiprocess-` entry removed so GDB treats the connection as multi-target capable.
    *Trace:* [SDD Section 5.1](SDD.md), [SDD Section 5.3.1](SDD.md).

*   <a id="HLR-059"></a>**HLR-059: Protocol Cleanup — qC, qOffsets, T, R.**
    The application shall stop replying with the empty packet for the standard small queries that `avarice` answers but Phase 1–8 leaves unimplemented, because some IDE configurations promote the resulting `unsupported feature` warnings to fatal errors. Specifically: `qC` shall return `QC<tid>` where `<tid>` is the currently selected `c`-thread (or `0` when no thread is selected); `qOffsets` shall return `Text=0;Data=0;Bss=0` because the AVR reset vector is fixed at address zero and no position-independent loading is performed; `T<tid>` (is-thread-alive) shall return `OK` for any `<tid>` present in the current `FsmContext.threads[]` array and `E01` otherwise; and `R<XX>` (RCmd restart) shall behave as `monitor reset` followed by `c` and reply with the resulting stop packet. None of these handlers shall alter execution state beyond the explicit semantics above.
    *Trace:* [SDD Section 5.3.1](SDD.md).

*   <a id="HLR-060"></a>**HLR-060: Multiprocess Thread-ID Parsing.**
    Because the application advertises `multiprocess+` in its `qSupported` reply (HLR-058), every RSP packet whose payload contains a thread-id field shall accept the GDB multiprocess form `p<PID>.<TID>` in addition to the legacy bare-hex form. Specifically: `Hg<tid>`, `Hc<tid>`, `Hs<tid>`, `T<tid>` (is-thread-alive), `qThreadExtraInfo,<tid>`, and any `:<tid>` action target inside a `vCont` packet shall be parsed by a single helper that recognises (a) bare hex `<TID>` (the GDB process id is implied to be the server's process id), (b) signed `-1` decoded as "all threads" / "any thread", (c) bare `0` decoded as "any thread", (d) `p<PID>.<TID>` with both fields hex, (e) `p0.0`, `p-1.-1`, `p<PID>.0`, and `p<PID>.-1` all decoded as "any thread in that process". The application shall use a fixed compile-time process-id constant `RSP_PID` (currently `0xa410`) for every reply that needs to emit a `<pid>` field; the value shall not be read from any uninitialised storage. Packets that fail this expanded parse shall reply `E01` exactly as before; packets that parse successfully but reference a TID not present in `FsmContext.threads[]` shall reply `E01` for `Hg`/`Hc`/`Hs`/`T` and the empty packet for `qThreadExtraInfo`. The implementation shall preserve the layered architecture (`SDD` §2.2): the parser lives entirely in `src/gdb_rsp.c` and the FSM-context lookup goes through the existing `RspContext.fsm` pointer.
    *Trace:* [SDD Section 5.3.1](SDD.md).

*   <a id="HLR-061"></a>**HLR-061: qThreadExtraInfo with FSM Name and State.**
    The `qThreadExtraInfo,<tid>` reply shall encode a human-readable label that lets a GDB front-end (e.g. VS Code's Call Stack view) distinguish individual avrOS FSMs at a glance. The label payload shall be the ASCII string `"FSM <name> [<active|quiescent>] state=0x<state_fn_hex>"` where `<name>` is the `FsmThread.name` field populated by `fsm_build_thread_list()` from the avrOS FSM table (HLR-013), `<active|quiescent>` reflects the `FsmThread.is_active` flag at the time the FSM context was last built, and `<state_fn_hex>` is the four-hex-digit FLASH word address of the FSM's next dispatch entry-point read from FSM_TABLE+offset (LLR-FSM-09). The label shall be hex-encoded byte-by-byte per the RSP spec and replied as a normal packet. The avrOS runtime is a cooperative run-to-completion FSM scheduler: every FSM executes on the **single shared hardware stack** set up by the C-runtime init code (`__stack` symbol from the linker map, growing down from `RAMEND`). At any halt, only the currently-running FSM has a live call stack; the other N−1 FSMs are quiescent between dispatches and have no preserved register frame anywhere in SRAM. The `[quiescent]` suffix therefore signals to the user that GDB will produce a one-frame backtrace for that FSM — the FSM `state` function — which is documented behaviour, not a bug. Unknown TIDs shall reply with the empty packet (signalling "unknown" per the RSP spec) rather than `E01`. The total reply payload shall not exceed `PacketSize=800`/2 = 400 bytes; FSM names longer than 32 bytes shall be silently truncated.
    *Trace:* [SDD Section 5.3.1](SDD.md).
