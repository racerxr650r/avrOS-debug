...
## 12. GDB Protocol Completion (avarice Drop-In Parity)

Requirements in this section govern the Phase 10 features that bring `avrOSdb`'s GDB Remote Serial Protocol surface up to functional parity with the legacy `avarice` JTAG/dW stub, so that any GDB script, IDE configuration, or `.gdbinit` written for `avarice` works unchanged against `avrOSdb` over UPDI. Tracked as GitHub issue #31; implements the design narrative from the Phase 10 overview paragraph in the SDD.
...
*   <a id="HLR-056"></a>**HLR-056: Hardware Data Watchpoints.**
    The application shall implement GDB watchpoint packets `Z2`/`z2` (write), `Z3`/`z3` (read), and `Z4`/`z4` (access) by programming the AVR-Dx OCD data-address comparator pair `DABP0`/`DABP1`. Up to two simultaneous watchpoints shall be supported, mirroring the two-comparator hardware limit; a third unique-address insert shall reply `E08`. A per-session shadow array `RspContext.hw_wp[]` shall record `(addr, length, kind)` for each programmed comparator so `z*` is exact and detach restores silicon to the unarmed state. The new UPDI primitives `updi_ocd_set_data_bp(slot, addr, length, kind)` and `updi_ocd_clear_data_bp(slot)` shall live alongside the existing `updi_ocd_set_hw_bp()` / `_clear_hw_bp()` so the OCD register accesses are kept in one module. On a watchpoint trigger the stop-reply packet shall carry the standard GDB stop-key `watch:<addr>;` (write), `rwatch:<addr>;` (read), or `awatch:<addr>;` (access) appended to the `T05thread:<id>;` payload so the IDE highlights the responsible address; the address shall be the GDB-side data-space address (UPDI byte address ORed with `0x800000`).
    *Trace:* [SDD Section 5.3.1](SDD.md), [SDD Section 4.3.1](SDD.md).

*   <a id="HLR-057"></a>**HLR-057: Memory-Map and Target Description XML.**
    The application shall implement the `qXfer:memory-map:read` and `qXfer:features:read:target.xml` annexed transfer packets so that GDB ceases to fall back to its built-in `avr` defaults and instead receives a runtime-generated description tailored to the device selected by HLR-048. The advertised feature string returned from `qSupported` shall be extended with `qXfer:memory-map:read+;qXfer:features:read+`. The memory map shall list FLASH (`type="flash" blocksize=<page_size>`), SRAM (`type="ram"`), EEPROM, USERROW, FUSES, LOCK, and SIGROW (`type="rom"`) regions using the addresses already published in `g_device_table[]`, expressed in the GDB unified address space (bit 23 set for data-space windows). The target description shall declare a single `<feature name="org.gnu.gdb.avr.cpu">` group enumerating R0–R31, SREG, SP, and PC with their canonical sizes and types, matching what `avr-gdb` synthesises today but explicit so the server is correct against non-AVR-aware GDB builds. Both transfer payloads shall be generated lazily on first request and cached for the lifetime of the session.
    *Trace:* [SDD Section 5.3.1](SDD.md), [SDD Section 5.1](SDD.md).
