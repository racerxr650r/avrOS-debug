...
Requirements for the manual-only on-target hardware integration test program. The harness links against `src/updi.c` (and, for Group D, spawns the production `build/avr-updi-gdb` binary via `fork`/`execl`) and runs against live AVR-Dx silicon connected via a UART/UPDI adapter.
