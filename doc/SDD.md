...
**Planned — Phase 10 — GDB Protocol Completion / avarice Drop-In Parity (not yet implemented).** The Phase 1–8 RSP server implements the minimum subset of the GDB Remote Serial Protocol needed to attach, read state, single-step, set up to two hardware breakpoints, continue, interrupt, and detach. Phase 10 closes the remaining gap between `avrOSdb` and the legacy `avarice` JTAG/dW stub so that any tool, script, IDE configuration, or `.gdbinit` written against `avarice` works unchanged against `avrOSdb` over UPDI. Tracked in GitHub issue #31; bound to HLRs in §12 below.
...
* **Hardware data watchpoints (`Z2`/`Z3`/`Z4`, `z2`/`z3`/`z4`).** Exposes the AVR-Dx OCD data-address comparators (`DABP0`, `DABP1`) as GDB write (`Z2`), read (`Z3`), and access (`Z4`) watchpoints. Implementation lives in `src/updi.c` (new `updi_ocd_set_data_bp()` / `_clear_data_bp()` primitives) and `src/gdb_rsp.c` (new `on_insert_wp`/`on_remove_wp` handler slots). Watchpoint hits shall be reported in the stop-reply packet with the standard `T05watch:<addr>;` / `rwatch` / `awatch` keys so the IDE displays the offending address.
...
* **Memory-map and target-description XML (`qXfer:memory-map:read`, `qXfer:features:read`).** Phase 1–8 returns the empty packet for both, forcing GDB to fall back to its built-in `avr` defaults. Phase 10 ships a runtime-generated XML map that reflects the selected device family (HLR-048): FLASH base / size, SRAM base / size, EEPROM, USERROW, FUSES, LOCK, SIGROW windows. This enables `gdb load` to pick the right NVM controller automatically and lets the IDE colour memory views by region. Implementation reuses `g_device_table[]`.
