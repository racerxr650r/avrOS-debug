...
> **Status: 🔲 In progress — issue [#57](https://github.com/racerxr650r/avrOS-debug/issues/57), branch `57-phase-20-debug-in-sleep`.** Implemented: `ASI_SYS_CTRLA_CLKREQ` + `updi_set_debug_in_sleep`/`updi_apply_debug_in_sleep` (`src/updi.c/.h`); `updi_enter_debug()` asserts CLK_REQ on the STOPPED success path by default (every entry incl. post-reset); `--sleep` CLI flag disables it (`src/main.c`, usage + man page). Spec via **tracer**: **HLR-077** + **LLR-UPDI-35** + **LLR-MAIN-25** + STP; rendered, lint 0/0. Unit tests: `updi_apply_debug_in_sleep` emits/omits the CLK_REQ STCS over PTY; `parse_args --sleep`. `make` 0 warnings; `make test` 10/10. **Hardware sanity:** `make hw-test-dap` (which now drives `updi_enter_debug` + CLK_REQ on the AVR128DA28) stays green — OCD entry is not regressed. Remaining gate: bench-confirm the original bug — set a breakpoint past an avrOS `sysSleep()` (the `main.c:139` case) and verify it now fires after the firmware wakes.
