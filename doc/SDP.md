...
> **Status: 🔲 In progress — issue [#54](https://github.com/racerxr650r/avrOS-debug/issues/54), branch `54-phase-18-dap-breakpoints`.** Breakpoint policy extracted to a shared core `src/debug_bp.{h,c}` (used by both the RSP and DAP front-ends); GDB Group-G **25/25** — the eviction arbiter fixed G8 and the fresh-PC-skip fix (below) fixed the long-standing G19, a clean sweep (`develop` was 23/25). DAP `setBreakpoints` + DWARF CFI `stackTrace` are the remaining work.
