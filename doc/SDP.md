...
> **Status: 🚧 Planned.** Supersedes the FSM-as-GDB-thread design of Phase 11. To be filed as a GitHub issue (successor to #36) and developed on its own feature branch per §5.1. Requirement changes (retire/revise the HLRs listed below) shall be authored in `doc/Project.xml` via the `tracer` skill **before** any source change lands.
...
| 6 | RSP sequence-replay harness | Host-side fake target (flash bytes, PC, 2 HW comparators, BREAK semantics) replaying captured golden RSP sequences in CI — no hardware | `tests/test_rsp_seq.c` (new) | new HLR (test infra) |
...
5. **`tests/test_rsp_seq.c` — host-side RSP sequence-replay harness (new test-infra HLR). [step B of the diagnosis plan].** A programmable fake target (models flash words, PC, the two OCD comparators, and BREAK-on-execute) driven by **captured golden RSP transcripts** committed under `tests/fixtures/rsp/`. The headline fixture replays the exact 2026-06-02 crashing sequence — `monitor reset; tbreak main; continue; break *0x3fa; continue; bt` — and asserts the post-demote wire is correct: `qfThreadInfo → m1`, a single-thread backtrace, **no degenerate frames, no all-`0xFF` label**. Wired into `make test` so CI catches any regression of this class without hardware.

6. **Capture & commit golden fixtures.** Record one `avr-gdb -batch` transcript and one `cortex-debug` (`showDevDebugOutput: raw`) transcript against the avrOS example, post-demote; commit as the replay fixtures so future breakpoint changes are pinned against real front-end sequences.
...
**Acceptance.**
- **Crash class gone (hardware, `hw-test-gdb`):** load avrOS firmware, set a breakpoint, hit it, run `bt` — avr-gdb returns a clean single-thread backtrace and does **not** crash; `detach` is clean. (This is the scenario that produced the core dump.)
...
- **CI net (host, `make test`):** the new `test_rsp_seq` replays the captured crashing transcript green; `qfThreadInfo` returns only TID 1; no FSM register-frame path remains.
