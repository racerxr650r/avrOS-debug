...
**Depends on:** Phases 0–5 complete (binary `avrOSdb` builds clean, all 102 unit + 4 integration tests pass).
...
- [ ] `make install` — installs binary to `$(PREFIX)/bin/avrOSdb` (mode 0755) and man page to `$(PREFIX)/share/man/man1/avrOSdb.1` (mode 0644). Default `PREFIX=/usr/local`. Creates intermediate dirs via `install -d`.
...
- [ ] `make bundle` — produces under `dist/`:
  - [ ] `dist/avrOSdb_$(VERSION)_amd64.deb` via `dpkg-deb` (with `DEBIAN/control` declaring Package, Version, Architecture: amd64, Maintainer, Description).
  - [ ] `dist/avrOSdb-$(VERSION)-1.x86_64.rpm` via `rpmbuild` (generated `.spec` with Name, Version, Release, Summary, License, `%install`, `%files`).
...
- [ ] `doc/avrOSdb.1` — `groff` man page with sections: NAME, SYNOPSIS, DESCRIPTION, OPTIONS, OPERANDS, EXIT STATUS, EXAMPLES, SEE ALSO (`avr-gdb(1)`, `avrdude(1)`). Must parse cleanly via `man -l doc/avrOSdb.1`.
